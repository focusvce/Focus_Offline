...
import com.hybrid.mybackend.Sqlite.DatabaseOperations;
...
public class TopMainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sharedPreferences = getSharedPreferences("com.hybrid.mybackend",MODE_PRIVATE);
        ...
        if(sharedPreferences.getBoolean("firstrun",true)){
            ...
            Cursor cr = dp.getInfo_Post(dp);
            if (cr.getCount() != 0) {
                cr.moveToFirst();
                do {
                    NewsFeed newsFeed = new NewsFeed(cr.getString(0),cr.getString(1),cr.getString(2),cr.getString(3),
                            cr.getString(4),cr.getString(5),cr.getInt(6),cr.getInt(7),cr.getInt(8),cr.getInt(9));
                    newsFeeds.add(newsFeed);
                } while (cr.moveToNext());
                recyclerAdapterMain.setFeed(newsFeeds);
            }
            sendJsonrequest();
        }
        ...
        if(response != null && response.length()!=0) {
            ...
            try {
                for (int i = response.length()-1; i>=0; i--) {
                    ...
                    dp.putInfo_Post(dp,title,link,description,dop,user_name,user_pic,sr_key,up,comment_count,uid);
